...
Wait4keyboard:
	lb $t1, 0($k1)
	bne $t1, 0, keyboardIntr		#Check key ready
	nop
	li $v0, 32	#Sleep
	li $a0, 5
	syscall
...
keyboardIntr:
	...
	nop
	b Wait4keyboard		# Khi return từ ctrinh ngắt thì kiểm tra tiếp char tiếp theo nhập vào
	nop
...
checkIntr_Cause:
	li $t1, MASK_CAUSE_COUNTER	#Kiểm tra nguyên nhân ngắt
	mfc0 $t2, $13			# $t2 chứa thông tin nguyên nhân ngắt
	bne $t1, $t2, check_string	#Check không phải do Counter thì check_string
	nop
	
counterIntr:
	blt $s4, 40, continue3		#40 lần ngắt tương đương 40 lần ngủ 5ms -> tổng là 200 ms ngủ
	nop				#40 lần ngắt tương đương 1 giây chương trình
	jal show_7segsLed
	nop
